 ...
 */
public class LicenseKeyboardUtil extends BaseActivity {
    private Context ctx;
    ...
    public LicenseKeyboardUtil(Context ctx, EditText edits[], boolean flag) {
        if (flag == false) {
            max = 7;
        } else {
            max = 8;
        ...
        keyboard = ((Activity) ctx).findViewById(R.id.keyboard);
    ...
    private OnKeyboardActionListener listener = new OnKeyboardActionListener() {
        ...
        public void onPress(int primaryCode) {
            ...
            if (primaryCode == 112) { //xml中定义的删除键值为112
                ...
                intent.putExtra(AddVehicleActivity.INPUT_LICENSE_KEY, license);
                intent.setAction(AddVehicleActivity.INPUT_LICENSE_COMPLETE);
                ...
                if (currentEditText == 0) {
                    ...
                    currentEditText++;
                    if (currentEditText >= max) {
                        currentEditText = max - 1;
                    }
                }
            }
        }
